...
/// The [RPTask] abstract class defines a task to be carried out by a participant
/// in a research study.
///
/// Extend this protocol to enable dynamic selection of the steps for a given
/// task.
/// By default, [RPOrderedTask] extends this abstract class for simple
/// sequential tasks.
/// Each step ([RPStep]) in a task roughly corresponds to one screen through
/// their [stepWidget] Widget, and represents the primary unit of work in any
/// task presented by a task view controller.
abstract class RPTask {
  final String _identifier;

  RPTask(this._identifier, {this.closeAfterFinished});

  /// A unique identifier of the Task. This identifier connects the Task to
  /// its result ([RPTaskResult]) object.
...
//  RPTaskProgress getProgressOfCurrentStep(RPStep step, RPTaskResult result);
  ...
  /// If set to `true` the Task will close after the participant has
  /// finished the task. If it's set to `false` no navigation function is
  /// called.
  ///
...
/// Simple class for keeping track the progress of the task.
/// It contains the number of the current step and the total number of steps.
///
